...
const pickRandomPrompts = (count: number): string[] => {
...
export const useRandomPrompts = (count: number = 3) => {
  const [prompts, setPrompts] = useState(() => pickRandomPrompts(count));

  const shuffle = useCallback(() => {
    setPrompts(pickRandomPrompts(count));
  }, [count]);
